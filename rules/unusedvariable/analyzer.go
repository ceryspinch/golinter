...
var Analyzer = &analysis.Analyzer{
	Name:     "unusedvariable",
	Doc:      "Checks for variables that are declared at package level but never used",
	Run:      run,
...
func run(pass *analysis.Pass) (interface{}, error) {
	...
	// Go code will not compile if variables are declared but not used within functions,
				...
				if !isUsed {
					pass.Reportf(
						...
						(color.RedString("Variable %q has been declared but is not used, ", varName))+
							color.CyanString("which means that it is redundant. ")+
